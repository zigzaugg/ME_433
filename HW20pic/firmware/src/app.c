            ...
            if (gotRx) {
                ...
                if(rxVal != -1){
                    ...
                    if (error<0) { // slow down the left motor to steer to the left
                            error  = -error;
                            left = MAXDUTY - KP*error-KD*(error-elast);
                            right = MAXDUTY;
                            if (left < 0){
                                left = 0;
                            }
                    } else { // slow down the right motor to steer to the right
                        right = MAXDUTY - KP*error-KD*(error-elast);
                        left = MAXDUTY;
                        if (right<0) {
                            right = 0;
                        }
                    ...
                    LATAbits.LATA1 = 0; //left
                    LATBbits.LATB3 = 0; //right
                    OC1RS = right; //left //cheating, but works
                    OC4RS = left; //right
                }
